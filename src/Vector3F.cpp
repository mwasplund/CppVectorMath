...
{
	std::wostream& operator<<(std::wostream& stream, const Vector3F& value)
	{
	...
	{
		return Vector3Utils::Equal(m_values, rhs);
	}

	bool Vector3F::operator!=(const Vector3F& rhs) const
	{
		return Vector3Utils::NotEqual(m_values, rhs);
	}

	bool Vector3F::NearEqual(const Vector3F& rhs, const ScalarF& epsilon) const
	{
		return Vector3Utils::NearEqual(m_values, rhs, epsilon);
	}
